...
ray.init(runtime_env={"working_dir": "CardiacSegV2"})
...
def main_worker(args):
    ...
    if args.checkpoint is not None and os.path.exists(args.checkpoint):
        ...
        if args.model_name == "swinunetr" and args.ssl_checkpoint and os.path.exists(args.ssl_checkpoint):
            weight = torch.load(args.ssl_checkpoint)
            state_dict = weight["state_dict"]

            # 如果 checkpoint 用 DataParallel 可能會有 module. 前綴
            if list(state_dict.keys())[0].startswith("module."):
                print("Removing 'module.' from state_dict keys")
                state_dict = {k.replace("module.", ""): v for k, v in state_dict.items()}

            # 如果原本是 swin_vit 命名方式，改成 SwinUNETR 內部命名
            if list(state_dict.keys())[0].startswith("swin_vit"):
                print("Replacing 'swin_vit' with 'swinViT'")
                state_dict = {k.replace("swin_vit", "swinViT"): v for k, v in state_dict.items()}

            # 載入權重到模型，strict=False 允許只載入 encoder 部分
            model.load_state_dict(state_dict, strict=False)
            print(f"Loaded SSL pretrained weights for {args.model_name} from {args.ssl_checkpoint}")

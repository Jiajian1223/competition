...
from CardiacSegV2.data_utils.utils import get_pids_by_loader, get_pids_by_data_dicts
...
def main_worker(args):
    ...
    ce = nn.CrossEntropyLoss(
        weight=torch.tensor(class_weights).float().to(args.device)
    )

    dice_loss = 0.3 * dice + 0.7 * ce
